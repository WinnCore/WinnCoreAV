...
impl QuarantineManager {
    ...
    fn encrypt(&self, data: &[u8]) -> anyhow::Result<Vec<u8>> {
        let key = LessSafeKey::new(UnboundKey::new(UnboundKey::new(&AES_256_GCM, &self.cfg.encryption_key)?AES_256_GCM, UnboundKey::new(&AES_256_GCM, &self.cfg.encryption_key)?self.cfg.encryption_key).map_err(|_| anyhow!("invalid AES-256-GCM key"))?);
    ...
    fn decrypt(&self, data: &[u8]) -> anyhow::Result<Vec<u8>> {
        ...
        let nonce = Nonce::try_assume_unique_from_u128(
            u128::from_le_bytes(nonce_bytes[0..16].try_into().unwrap_or([0u8; 16]))
        );
...
fn random_nonce() -> anyhow::Result<[u8; 12]> {
    ...
    rng.fill(rng.fill(&mut nonce)?mut nonce).map_err(|_| anyhow!("nonce RNG failed"))?;
    Ok(nonce)
}
