//! WinnCore Filesystem Monitoring Library
//!
//! Production monitoring with owned runtime for filesystem events and malware scanning.
    ...
    fn queue_scan(&self, path: &Path) -> Result<()> {
        ...
        if let Ok(tx_guard) = self.tx.lock() {
            if let Some(ref tx) = *tx_guard {
                // Explicitly type annotate to fix E0282
                let sender: &Sender<PathBuf> = tx;
                if sender.try_send(real_path.clone()).is_err() {
